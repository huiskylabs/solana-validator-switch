...
impl<'a> StatusApp<'a> {
    ...
    async fn fetch_vote_data_only(&mut self) {
        ...
        for validator_status in self.app_state.validator_statuses.iter() {
            ...
            if let (Some(ref data), Some(old_slot)) = (&vote_data, old_slots.get(index).and_then(|&x| x)) {
                if let Some(last_vote) = data.recent_votes.last() {
    ...
    async fn fetch_catchup_data_only(&mut self) {
        ...
        for validator_status in self.app_state.validator_statuses.iter() {
            ...
            if validator_status.nodes_with_status.len() >= 2 {
                // Node 0 catchup
                catchup_statuses.0 = self.fetch_catchup_for_node(&validator_status.nodes_with_status[0]).await;
                // Node 1 catchup
                catchup_statuses.1 = self.fetch_catchup_for_node(&validator_status.nodes_with_status[1]).await;
            }
    ...
    async fn fetch_catchup_for_node(&self, node: &crate::types::NodeWithStatus) -> Option<CatchupStatus> {
        ...
        match pool.execute_command_with_early_exit(
            ...
            Ok(output) => {
                if output.contains("0 slot(s)") || output.contains("has caught up") {
                    Some(CatchupStatus { status: "Caught up".to_string() })
                    ...
                    if let Ok(slots) = slots_str.parse::<u64>() {
                        Some(CatchupStatus { status: format!("{} slots behind", slots) })
                    } else {
                        Some(CatchupStatus { status: "Checking...".to_string() })
                    }
                } else {
...
fn ui(f: &mut ratatui::Frame, app: &StatusApp) {
    ...
    for (index, ((((validator_status, vote_data), catchup_data), prev_slot), inc_time)) in app.app_state.validator_statuses.iter()
        .zip(app.vote_data.iter())
        .zip(app.catchup_data.iter())
        .zip(app.previous_last_slots.iter().chain(std::iter::repeat(&None)))
        .zip(app.increment_times.iter().chain(std::iter::repeat(&None)))
        .enumerate()
    {
        if index < content_chunks.len() {
            render_validator(f, content_chunks[index], validator_status, vote_data.as_ref(), catchup_data, index, *prev_slot, *inc_time);
        }
    }
...
fn render_validator(
    ...
    if let Some(ref metadata) = validator_status.metadata {
        if let Some(ref name) = metadata.name {
            info_lines.push(Line::from(vec![
                ...
                Span::raw(" "),
                Span::styled(name.to_uppercase(), Style::default().fg(Color::White).add_modifier(Modifier::BOLD)),
                Span::raw(" "),
                Span::styled("⚡", Style::default().fg(Color::Yellow)),
                Span::raw(" ━━━"),
            ]));
            ...
            info_lines.push(Line::from(vec![
                Span::raw("  🗳️  Vote: "),
                Span::styled(truncate_string(&validator_pair.vote_pubkey, 44), Style::default().fg(Color::Cyan)),
            ]));
            info_lines.push(Line::from(vec![
                Span::raw("  🆔 Identity: "),
                Span::styled(truncate_string(&validator_pair.identity_pubkey, 44), Style::default().fg(Color::Magenta)),
            ]));
        }
    } else {
        info_lines.push(Line::from(vec![
            ...
            Span::styled("⚡", Style::default().fg(Color::Yellow)),
            Span::raw(" "),
            Span::styled(format!("VALIDATOR {}", index + 1), Style::default().fg(Color::White).add_modifier(Modifier::BOLD)),
            Span::raw(" "),
            Span::styled("⚡", Style::default().fg(Color::Yellow)),
            Span::raw(" ━━━"),
        ]));
    ...
    if validator_status.nodes_with_status.len() >= 2 {
        ...
        render_status_table(f, chunks[1], node_0, node_1, validator_status, vote_data, catchup_data, previous_last_slot, increment_time);
    }
}
...
fn render_status_table(
        ...
        Cell::from(vec![
            Line::from(vec![
                ...
                Span::styled(
                    ...
                    Style::default().fg(if node_0.status == crate::types::NodeStatus::Active { Color::Green } else { Color::Yellow }).add_modifier(Modifier::BOLD)
                ),
            ]),
            Line::from(vec![
                Span::raw("   "),
        ...
        Cell::from(vec![
            Line::from(vec![
                ...
                Span::styled(
                    ...
                    Style::default().fg(if node_1.status == crate::types::NodeStatus::Active { Color::Green } else { Color::Yellow }).add_modifier(Modifier::BOLD)
                ),
            ]),
            Line::from(vec![
                Span::raw("   "),
    ...
    rows.push(Row::new(vec![
        ...
        Cell::from(format!(" {} {}", node_0_status_icon, node_0_label)).style(Style::default().fg(node_0_color).add_modifier(Modifier::BOLD)),
        Cell::from(format!(" {} {}", node_1_status_icon, node_1_label)).style(Style::default().fg(node_1_color).add_modifier(Modifier::BOLD)),
    ]));

    // Version row with icon
    let node_0_version = node_0.version.as_ref().cloned().unwrap_or("N/A".to_string());
    let node_1_version = node_1.version.as_ref().cloned().unwrap_or("N/A".to_string());
    rows.push(Row::new(vec![
    ...
    let node_0_sync = node_0.sync_status.as_ref().cloned().unwrap_or("Unknown".to_string());
    let node_1_sync = node_1.sync_status.as_ref().cloned().unwrap_or("Unknown".to_string());
    let node_0_sync_icon = if node_0_sync.contains("Caught up") { "🔄" } else if node_0_sync == "Unknown" { "⚠️" } else { "⏳" };
    let node_1_sync_icon = if node_1_sync.contains("Caught up") { "🔄" } else if node_1_sync == "Unknown" { "⚠️" } else { "⏳" };
    let node_0_sync_color = if node_0_sync.contains("Caught up") { Color::Green } else if node_0_sync == "Unknown" { Color::Red } else { Color::Yellow };
    let node_1_sync_color = if node_1_sync.contains("Caught up") { Color::Green } else if node_1_sync == "Unknown" { Color::Red } else { Color::Yellow };
    rows.push(Row::new(vec![
        Cell::from(" → Sync").style(Style::default().fg(Color::Gray)),
        Cell::from(format!(" {} {}", node_0_sync_icon, truncate_sync_status(&node_0_sync, 35))).style(Style::default().fg(node_0_sync_color)),
        Cell::from(format!(" {} {}", node_1_sync_icon, truncate_sync_status(&node_1_sync, 35))).style(Style::default().fg(node_1_sync_color)),
    ]));

    // Identity row with key icon
    let node_0_identity = node_0.current_identity.as_ref().cloned().unwrap_or("Unknown".to_string());
    let node_1_identity = node_1.current_identity.as_ref().cloned().unwrap_or("Unknown".to_string());
    rows.push(Row::new(vec![
        Cell::from(" → Identity").style(Style::default().fg(Color::Gray)),
        Cell::from(format!(" 🔑 {}", truncate_string(&node_0_identity, 25))).style(Style::default().fg(Color::White)),
        Cell::from(format!(" 🔑 {}", truncate_string(&node_1_identity, 25))).style(Style::default().fg(Color::White)),
    ]));

    // Ledger path row with folder icon
    let node_0_ledger = node_0.ledger_path.as_ref()
        .map(|p| truncate_path(p, 35))
        .unwrap_or("N/A".to_string());
    let node_1_ledger = node_1.ledger_path.as_ref()
        .map(|p| truncate_path(p, 35))
        .unwrap_or("N/A".to_string());
    rows.push(Row::new(vec![
    ...
    rows.push(Row::new(vec![
        Cell::from(" 🔄 LIVE DATA").style(Style::default().fg(Color::Magenta).add_modifier(Modifier::BOLD)),
        Cell::from(""),
        Cell::from(""),
    ]));

    // Catchup status row
    let node_0_catchup = catchup_data.0.as_ref()
        .map(|c| c.status.clone())
        .unwrap_or("N/A".to_string());
    let node_1_catchup = catchup_data.1.as_ref()
        .map(|c| c.status.clone())
        .unwrap_or("N/A".to_string());
    ...
    let node_0_catchup_icon = if node_0_catchup.contains("Caught up") { "✓" } else if node_0_catchup == "N/A" { "✗" } else { "⌛" };
    let node_1_catchup_icon = if node_1_catchup.contains("Caught up") { "✓" } else if node_1_catchup == "N/A" { "✗" } else { "⌛" };
    
    rows.push(Row::new(vec![
        Cell::from(" → Catchup").style(Style::default().fg(Color::Gray)),
        Cell::from(format!(" {} {}", node_0_catchup_icon, truncate_sync_status(&node_0_catchup, 35))).style(Style::default().fg(node_0_catchup_color)),
        Cell::from(format!(" {} {}", node_1_catchup_icon, truncate_sync_status(&node_1_catchup, 35))).style(Style::default().fg(node_1_catchup_color)),
    ]));
    ...
    if let Some(vote_data) = vote_data {
        ...
        let voting_color = if vote_data.is_voting { Color::Green } else { Color::Yellow };
        ...
        rows.push(Row::new(vec![
            ...
            Cell::from(format!(" {}", voting_status)).style(Style::default().fg(voting_color).add_modifier(Modifier::BOLD)),
            votes_cell,
        ]));
    } else {
