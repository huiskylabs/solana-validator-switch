...
mod startup_validation_tests {
    use crate::types::{NodeConfig, NodePaths, NodeStatus, NodeWithStatus, ValidatorPair, ValidatorType};
    ...
    fn create_test_node(label: &str, host: &str, available: bool) -> NodeWithStatus {
        NodeWithStatus {
            ...
            status: if available { NodeStatus::Standby } else { NodeStatus::Unknown },
    ...
    fn test_startup_continues_when_unrelated_validator_down() {
        ...
        assert_eq!(warnings[0], "Validator 2 Node 2 (node-2-2): SSH connection failed (will limit functionality)");
        assert!(!has_critical_failures);
        
        // Verify that Validator 1 nodes are both ready
        assert_eq!(validator_statuses[0].nodes_with_status[0].swap_ready, Some(true));
        assert_eq!(validator_statuses[0].nodes_with_status[1].swap_ready, Some(true));
    }
    ...
    fn test_startup_continues_when_source_node_down() {
        ...
        assert!(warnings.iter().any(|w| w.contains("node-1-1") && w.contains("SSH connection failed")));
        assert!(!has_critical_failures);
    ...
    fn test_old_behavior_would_have_blocked() {
        ...
        for (validator_idx, validator_status) in validator_statuses.iter().enumerate() {
            for (node_idx, node_with_status) in validator_status.nodes_with_status.iter().enumerate() {
                ...
                if node_with_status.status == NodeStatus::Unknown {
                    critical_failures_old.push(format!(
