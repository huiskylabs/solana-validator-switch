...
impl StartupLogger {
    ...
    pub fn new() -> Result<Self> {
        ...
        logger.log(&format!("Started at: {}", Local::now().format("%Y-%m-%d %H:%M:%S")))?;
    ...
    pub fn log_ssh_command(&self, host: &str, command: &str, output: &str, error: Option<&str>) -> Result<()> {
    ...
    pub fn create_latest_symlink(&self) -> Result<()> {
        let latest_path = self.log_path.parent()
            .unwrap()
            .join("latest.log");
